...
function! s:gReplace_show_matches()
    ...
    command! -buffer -nargs=0 -bang Gvisit call s:gVisit()

    nnoremap <CR> :Gvisit<CR>

    let s:save_qf_list = new_qf
endfunction

function! s:gVisit()
    let G_line_number = line('.')
    let G_col = col('.')
    let file_info = matchstr(getline('.'), '.\{-}:\d\+')
    let file_path = matchstr(file_info, '.\{-}\ze:')
    let file_name_goto_line = substitute(file_info, file_path.':', '', 'g')

    :wincmd k

    exec ':e '.file_path
    call cursor(file_name_goto_line, 0)

    :wincmd j
    call cursor(G_line_number, G_col)
endfunction
...
function! s:gSearch(type, ...)
    ...
    if a:type == 'grep'
        if filenames == ''
            let filenames = input('Search in files: ', g:grep_default_file_names, 'file')
        endif
    elseif a:type == 'args'
